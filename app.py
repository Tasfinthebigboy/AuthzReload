...
async def on_member_join(member):
    try:
        ...
        # Fetch the background image from the provided URL
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(background_url) as background_response:
                ...
                background_image = background_image.resize((800, 450))  # Resize to fit the canvas
        ...
        # Convert to static image if it's animated (GIF)
        ...
        # Create a canvas and paste the background image
        canvas = Image.new("RGBA", background_image.size, "#2C2F33")
        canvas.paste(background_image, (0, 0))
        draw = ImageDraw.Draw(canvas)

        # Paste the avatar image at the center of the canvas
        ...
        font_path = "DejaVuSans-Bold.ttf"  # Ensure this font is available
        ...
        # Save the welcome image to a BytesIO object
        output_buffer = BytesIO()
        canvas.save(output_buffer, format="PNG")
        output_buffer.seek(0)

        # Check for welcome channel and send the image
